...
contract Avaxcoins is ERC721A, Ownable, ReentrancyGuard, ERC2981 {
    using Strings for uint256;
    ...
    uint96 public tokenRoyalties = 600; // 7% royalty
    uint256 private preMintCounter = 0;
    ...
    constructor() ERC721A("AvaxCoins", "AVAXCOINS") {
    ...
    function preMint(address[] memory addresses) external onlyOwner {
        require(addresses.length + preMintCounter <= MAX_SUPPLY, "MiniMonkeez: premint is complete");
        for (uint256 i = 0; i < addresses.length; i += 1) {
            _safeMint(addresses[i], 1);
        }
        preMintCounter += addresses.length;
    }

    function mint(address to) external nonReentrant returns (bool) {
        require(false, "Avaxcoins: Minting complete");
        return true;
    }
